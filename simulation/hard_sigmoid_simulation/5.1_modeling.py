...
for num_layer in range(5,21):
    for lr in range(-3,-6,-1):
        ...
        if os.path.exists(f'./NNs/HS_{exp_setup}.model'):
            print(f'./NNs/HS_{exp_setup} exists.')
        else:

            a = torch.load('./data/hard_sigmoid.ds')
            ...
            torch.save(model, f'./NNs/HS_{exp_setup}.model')
