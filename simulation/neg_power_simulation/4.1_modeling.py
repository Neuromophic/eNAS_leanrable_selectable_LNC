...
for num_layer in range(3,11):
    for lr in range(-3,-6,-1):
        ...
        if os.path.exists(f'./NNs/NP_{exp_setup}.model'):
            print(f'NP_{exp_setup} exists.')
        else:
            
            a = torch.load('./data/neg_power.ds')
            ...
            torch.save(model, f'./NNs/NP_{exp_setup}.model')
