...
class PrintedNeuromorphicCircuit:
    ...
    def act(self, node_gene, a):
        ...
        if activation_fn == 'ptanh':
            ...
            act.rt_.data = torch.tensor([node_gene.S_R1n, node_gene.S_R2n,
                                         node_gene.S_W1n, node_gene.S_L1n,
                                         node_gene.S_W2n, node_gene.S_L2n])
        else:
    ...
    def neg(self, node_gene, x):
        ...
        inv.rt_.data = torch.tensor([node_gene.NEG_R1n, node_gene.NEG_R2n, node_gene.NEG_R3n,
                                     node_gene.NEG_W1n, node_gene.NEG_L1n,
                                     node_gene.NEG_W2n, node_gene.NEG_L2n,
                                     node_gene.NEG_W3n, node_gene.NEG_L3n])
        return inv(x)
    ...
    def forward(self, data, config):
        ...
        for node, gb, gd, node_gene, inputs in self.node_evals:
            ...
            for i, g in inputs:
                ...
                value_temp = self.neg(node_gene, torch.tensor(self.values[i]).unsqueeze(-1))[:,:,0].detach().numpy() * (self.theta_ste(g, config) < 0) +\
            ...
            bias_temp = self.neg(node_gene, torch.ones(self.N, 1, 1))[:,:,0].detach().numpy() * (self.theta_ste_noisy(gb, config) < 0) + 1. *(self.theta_ste_noisy(gb, config) >= 0)
            ...
            self.values[node] = self.act(node_gene, z)[:,:,0].detach().numpy()
